...
int trait_num;
{
    ...
    for (i=2; i<templen; i++) {
        if (!strin(NAME_CHARS,tempstr[i])) {
	        sf(badstr,"illegal character '%c' in trait name %s", tempstr[i],tempstr);
	        baddata(badstr);
	    }
    }
    tempstr=name+1;
		    ...
		    /* get rid of comment after equation before sending it
		       to the equation parser (which is easily confused) */
          		...
          		if (did_we_do_eqn == 1) {
                    ...
                    sf(badstr,"error in reading trait information for trait %s", name);
                    baddata(badstr);
    		    }
    	    } else {
                trait[trait_num][j] = MISSING_PHENO;
            }
    	} else {
